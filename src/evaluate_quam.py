"""
Evaluate QUAM (Query Affinity Modelling) on statute retrieval datasets.

Methodology: Rathee et al., "Quam: Adaptive Retrieval through Query Affinity Modelling" (WSDM 2025)

Builds on GAR with SetAff frontier update:
...
Same infrastructure as GAR (BM25 corpus graph, BM25 initial pool, cross-encoder scorer),
only the adaptive re-ranking algorithm differs.
...
Usage:
  python src/evaluate_quam.py --dataset kuhperdata
  python src/evaluate_quam.py --dataset all --budget 100 --top_s 30
  python src/evaluate_quam.py --dataset kuhperdata --scorer bge
"""
...
def pre_score_pools(
    ...
    than the per-query-per-batch iterative loop.
    ...
    total = len(qids)
    n_queries = len(pools)
    print(f"  Pre-scoring {total:,} pairs ({n_queries} queries x ~{total // n_queries} docs) "
...
def make_scorer_fn(
    ...
    """
    Create a QUAM scorer closure for a single query.
...
def run_quam(
    ...
    quam = QUAM(
        corpus_graph=graph, budget=budget, batch_size=batch_size,
        top_s=top_s, backfill=True,
    )
...
def main():
    parser = argparse.ArgumentParser(description="QUAM evaluation on statute retrieval datasets")
    ...
    parser.add_argument("--initial_pool_size", type=int, default=None,
                        help="BM25 pool size fed to QUAM (default: per-dataset or 100)")
    parser.add_argument("--budget", type=int, default=None,
                        help="Max docs scored by the reranker (default: per-dataset or 100)")
    parser.add_argument("--batch_size", type=int, default=16,
                        help="Docs scored per QUAM iteration (paper default: 16)")
    parser.add_argument("--top_s", type=int, default=None,
                        help="Set S size for SetAff (top-s scored docs for frontier expansion). "
                             "Rule of thumb: ~0.2-0.3 * budget. Default: per-dataset or 30")
    ...
    for name, cfg in datasets.items():
        ...
        if args.dataset == "all":
            ...
            top_s = dset_defaults.get("top_s", _GLOBAL_DEFAULTS["top_s"])
        ...
        print(f"\n{'=' * 60}")
        print(f"  QUAM (SetAff): {name.upper()} ({args.split} split)")
        print(f"  Scorer: {scorer_name} ({SCORERS[scorer_name][1]})")
        print(f"  Pool: {initial_pool_size}, Budget: {budget}, "
              f"top_s: {top_s}, batch: {args.batch_size}")
        print(f"  stemmer={use_stemmer}, stopwords={remove_stopwords}")
        print(f"{'=' * 60}")
        ...
        # Step 1: BM25 corpus graph (shared cache with GAR)
        ...
        # Step 3: Pre-score initial pool in bulk (GPU only)
        ...
        # Step 4: QUAM with SetAff
        print(f"\n  Step {'3b' if pre_scores else '3'}: QUAM SetAff "
              f"(pool={initial_pool_size}, budget={budget}, "
              f"batch={args.batch_size}, top_s={top_s}, graph_k={args.graph_k})")
        t0 = time.time()
        results = run_quam(
            loader, graph, bm25_pools, scorer,
            top_k=args.top_k, budget=budget,
            batch_size=args.batch_size, top_s=top_s,
            graph_k_limit=args.graph_k_limit,
